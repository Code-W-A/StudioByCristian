...
export default function BookingWizard() {
  ...
  function validateStep() {
    ...
    if (step === 2 && !form.startsAt) return "Choose an available date and time."
    if (step === 3 && (!form.name || !form.email || !form.phone || !form.location || form.description.length < 20 || !form.budget || !form.timeline)) return "Complete all project and contact fields. The description should contain at least 20 characters."
    return ""
  }
  ...
  if (success) return (
    <div className="rounded-3xl bg-[#111] p-8 text-white md:p-12" role="status">
  ...
  return (
    <div className="rounded-3xl border border-black/10 bg-[#f6f4ef] p-5 shadow-sm sm:p-8 lg:p-10">
      <div className="mb-8 flex items-center justify-between gap-3" aria-label={`Step ${step} of 4`}>
        {[1, 2, 3, 4].map((item) => <div key={item} className={`h-1.5 flex-1 rounded-full ${item <= step ? "bg-black" : "bg-black/10"}`} />)}
      </div>
      <p className="text-xs uppercase tracking-[0.25em] text-black/50">Step {step} of 4</p>
      ...
      {step === 2 && <div className="mt-4">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row"><input className={fieldClass} type="date" value={form.date} min={new Date().toISOString().slice(0, 10)} onChange={(event) => update("date", event.target.value)} /><button type="button" onClick={loadSlots} disabled={busy} className="shrink-0 rounded-xl bg-black px-6 py-3 text-sm text-white disabled:opacity-50">{busy ? "Loading…" : "Show times"}</button></div>
        <div className="mt-5 grid gap-2 sm:grid-cols-2">{slots.map((slot) => <button type="button" key={slot.startsAt} onClick={() => update("startsAt", slot.startsAt)} className={`rounded-xl border p-3 text-sm ${form.startsAt === slot.startsAt ? "border-black bg-black text-white" : "border-black/15 bg-white"}`}>{slot.localLabel}</button>)}</div>
      </div>}
      ...
      <div className="mt-8 flex items-center justify-between gap-4">
        <button type="button" onClick={() => { setError(""); setStep((current) => Math.max(1, current - 1)) }} disabled={step === 1 || busy} className="inline-flex items-center gap-2 rounded-full border border-black/20 px-5 py-3 text-sm disabled:opacity-30"><ArrowLeft size={16} /> Back</button>
        {step < 4 ? <button type="button" onClick={next} className="inline-flex items-center gap-2 rounded-full bg-black px-6 py-3 text-sm text-white">Continue <ArrowRight size={16} /></button> : <button type="button" onClick={() => void submit()} disabled={busy} className="inline-flex items-center gap-2 rounded-full bg-black px-6 py-3 text-sm text-white disabled:opacity-50">{busy && <Loader2 size={16} className="animate-spin" />} Submit request</button>}
      </div>
    </div>
  )
}
