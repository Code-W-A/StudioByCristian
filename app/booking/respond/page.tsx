...
function BookingResponseContent() {
  ...
  useEffect(() => { async function respond() { if (!token) return setState("This response link is incomplete."); try { const result = await callFirebase<{ token: string }, { status: string }>("respondToBooking", { token }); setState(result.status === "confirmed" ? "Thank you. Your proposed consultation time is confirmed." : "The proposed time was declined. Our team has been notified.") } catch (reason) { setState(reason instanceof Error ? reason.message : "The response could not be processed.") } } void respond() }, [token])
